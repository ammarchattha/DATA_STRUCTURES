...
//    tree1.Delete(40);
	...
	cout<<endl<<"Pre Order Print (Root--left--Right)"<<endl;
  tree2.PreOrderTraversal();     
    return 0;
}
node* AVLTree::Insert(node* t,array[]  ){
	int val=array[count];
 if (t==NULL)
    {
         t= new node(val);
...
node* AVLTree::Insert1(node* r, int val){
 if (r==NULL)
    {
        node* t= new node(val);
        if (r==root){
...
node* AVLTree::PreOrderTraversal( node* r){
    ...
    PreOrderTraversal(r->left,t);

    PreOrderTraversal(r->right,t);
